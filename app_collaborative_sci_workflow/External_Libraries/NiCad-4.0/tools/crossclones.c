...
typedef	TLchar	linetable___x238[720000001];
static linetable___x238	linetable_lineText;
static TLint4	linetable_lineTextSize;
typedef	TLint4	linetable___x241[14400000];
static linetable___x241	linetable_lineTable;
...
TLstring	s;
{
    typedef	TLnat1	__x245[256];
    typedef	__x245	nat256;
    register TLnat4	h;
    register TLnat4	j;
    h = TL_TLS_TLSLEN(s);
    if (h > 255) {
...
typedef	TLint4	linetable___x252[10];
static linetable___x252	linetable_primes = 
    {1021, 2027, 4091, 8191, 16381, 32003, 65003, 131009, 262007, 524047};
typedef	TLboolean	linetable___x256[256];
static linetable___x256	linetable_spaceP;
...
TLstring	__x83;
{
	...
	{
	    TLchar	__x260[2];
	    if ((first > rawlinelength) || ((TL_TLS_TLSBX(__x260, (TLint4) first, line), !(linetable_spaceP[((TLnat4) TLCVTTOCHR(__x260))])))) {
		break;
	    };
	};
	first += 1;
	...
	{
	    TLchar	__x261[2];
	    if ((last < first) || ((TL_TLS_TLSBX(__x261, (TLint4) last, line), !(linetable_spaceP[((TLnat4) TLCVTTOCHR(__x261))])))) {
		break;
	    };
	};
	last -= 1;
    ...
    {
	TLstring	__x262;
	TL_TLS_TLSBXX(__x262, (TLint4) last, (TLint4) first, line);
	TLSTRASS(255, line, __x262);
    };
    {
	TLSTRASS(255, __x83, line);
	return;
    };
    /* NOTREACHED */
}
...
{
    {
	TLSTRASS(255, __x94, (* (TLstring *) &linetable_lineText[(linetable_lineTable[lineIndex]) - 1]));
	return;
    };
    /* NOTREACHED */
}
...
typedef	struct PC	__x267[7200000];
static __x267	pcs;
static TLint4	npcs;
static TLint4	firstnewpc;
typedef	linetable_LN	__x268[120000000];
static __x268	lines;
...
typedef	struct CP	__x269[24000000];
static __x269	clonepairs;
...
static void readpcs () {
	...
	for (i = 1; i <= 7200000; i++) {
	    ...
	    {
		TLstring	__x270;
		TL_TLS_TLSBXX(__x270, (TLint4) sfend, (TLint4) sfindex, sourceheader);
		(*pc).srcfile = linetable_install(__x270);
	    };
	    slindex = (TL_TLS_TLSIND(sourceheader, "startline=") + TL_TLS_TLSLEN("startline=")) + 1;
	    slend = TL_TLS_TLSIND(sourceheader, " endline=") - 2;
	    {
		TLstring	__x271;
		TL_TLS_TLSBXX(__x271, (TLint4) slend, (TLint4) slindex, sourceheader);
		(*pc).srcstartline = TL_TLS_TLSVSI(__x271, (TLint4) 10);
	    };
	    elindex = (TL_TLS_TLSIND(sourceheader, "endline=") + TL_TLS_TLSLEN("endline=")) + 1;
	    elend = TL_TLS_TLSLEN(sourceheader) - 2;
	    {
		TLstring	__x272;
		TL_TLS_TLSBXX(__x272, (TLint4) elend, (TLint4) elindex, sourceheader);
		(*pc).srcendline = TL_TLS_TLSVSI(__x272, (TLint4) 10);
	    };
	    (*pc).firstline = nlines;
	    for(;;) {
		TLstring	line;
		...
		{
		    TLstring	__x273;
		    linetable_strip(line, __x273);
		    TLSTRASS(255, line, __x273);
		};
		if (strcmp(line, "") != 0) {
		    nlines += 1;
		    if (nlines > 120000000) {
...
static void readnewpcs () {
	...
	for (i = firstnewpc; i <= 7200000; i++) {
	    ...
	    {
		TLstring	__x274;
		TL_TLS_TLSBXX(__x274, (TLint4) sfend, (TLint4) sfindex, sourceheader);
		(*pc).srcfile = linetable_install(__x274);
	    };
	    slindex = (TL_TLS_TLSIND(sourceheader, "startline=") + TL_TLS_TLSLEN("startline=")) + 1;
	    slend = TL_TLS_TLSIND(sourceheader, " endline=") - 2;
	    {
		TLstring	__x275;
		TL_TLS_TLSBXX(__x275, (TLint4) slend, (TLint4) slindex, sourceheader);
		(*pc).srcstartline = TL_TLS_TLSVSI(__x275, (TLint4) 10);
	    };
	    elindex = (TL_TLS_TLSIND(sourceheader, "endline=") + TL_TLS_TLSLEN("endline=")) + 1;
	    elend = TL_TLS_TLSLEN(sourceheader) - 2;
	    {
		TLstring	__x276;
		TL_TLS_TLSBXX(__x276, (TLint4) elend, (TLint4) elindex, sourceheader);
		(*pc).srcendline = TL_TLS_TLSVSI(__x276, (TLint4) 10);
	    };
	    (*pc).firstline = nlines;
	    for(;;) {
		TLstring	line;
		...
		{
		    TLstring	__x277;
		    linetable_strip(line, __x277);
		    TLSTRASS(255, line, __x277);
		};
		if (strcmp(line, "") != 0) {
		    nlines += 1;
		    if (nlines > 120000000) {
...
static TLint4	__x279;
static TLint4	__x278;
	...
	if (i <= __x283) {
	    for(;;) {
		{
			...
			for(;;) {
				...
				dpmatrix[((j) + __x279 * (i))] = (dpmatrix[((j - 1) + __x279 * (i - 1))]) + 1;
			    } else {
				if ((dpmatrix[((j) + __x279 * (i - 1))]) >= (dpmatrix[((j - 1) + __x279 * (i))])) {
				    dpmatrix[((j) + __x279 * (i))] = dpmatrix[((j) + __x279 * (i - 1))];
				} else {
				    dpmatrix[((j) + __x279 * (i))] = dpmatrix[((j - 1) + __x279 * (i))];
				};
			    };
			    if (j == __x284) break;
			    j++;
		...
		if ((i - (dpmatrix[((n) + __x279 * (i))])) > difflimit) {
		    return (0);
		};
		if (i == __x283) break;
		i++;
...
typedef	TLint4	__x288[24000000];
static __x288	clonepairmap;
...
static void showpairs () {
	...
	{
	    TLchar	__x294[2];
	    TL_TLS_TLSBX(__x294, (TLint4) slashindex, pcfile);
	    pcfchar = (* (TLchar *) __x294);
	};
    ...
    {
	TLstring	__x296;
	TL_TLS_TLSBXS(__x296, (TLint4) 0, (TLint4) (slashindex + 1), pcfile);
	TLSTRASS(255, systeminfo, __x296);
    };
	...
	{
	    TLchar	__x297[2];
	    TL_TLS_TLSBX(__x297, (TLint4) usindex, systeminfo);
	    sichar = (* (TLchar *) __x297);
	};
    ...
    {
	TLstring	__x299;
	TL_TLS_TLSBXX(__x299, (TLint4) (usindex - 1), (TLint4) 1, systeminfo);
	TLSTRASS(255, systemname, __x299);
    };
    {
	TLstring	__x300;
	TL_TLS_TLSBXX(__x300, (TLint4) (TL_TLS_TLSIND(systeminfo, ".xml") - 1), (TLint4) (usindex + 1), systeminfo);
	TLSTRASS(255, granularity, __x300);
    };
    TL_TLI_TLISSO ();
	...
	if (i <= __x301) {
		...
		{
		    TLstring	__x303;
		    linetable_gettext((linetable_LN) ((*pc1).info), __x303);
		    {
			TLstring	__x302;
			TL_TLS_TLSBXS(__x302, (TLint4) -1, (TLint4) 1, __x303);
			TL_TLI_TLISSO ();
			TL_TLI_TLIPS ((TLint4) 0, __x302, (TLint2) -1);
		...
		if (showsource) {
			...
			if (k <= __x304) {
				...
				{
				    TLstring	__x305;
				    linetable_gettext((linetable_LN) (lines[(((*pc1).firstline) + k) - 1]), __x305);
				    TL_TLI_TLISSO ();
				    TL_TLI_TLIPS ((TLint4) 0, __x305, (TLint2) -1);
				    TL_TLI_TLIPK ((TLint2) -1);
				};
				if (k == __x304) break;
				k++;
		...
		{
		    TLstring	__x307;
		    linetable_gettext((linetable_LN) ((*pc2).info), __x307);
		    {
			TLstring	__x306;
			TL_TLS_TLSBXS(__x306, (TLint4) -1, (TLint4) 1, __x307);
			TL_TLI_TLISSO ();
			TL_TLI_TLIPS ((TLint4) 0, __x306, (TLint2) -1);
			TL_TLI_TLIPS ((TLint4) 0, " pcid=\"", (TLint2) -1);
		...
		if (showsource) {
			...
			if (k <= __x308) {
				...
				{
				    TLstring	__x309;
				    linetable_gettext((linetable_LN) (lines[(((*pc2).firstline) + k) - 1]), __x309);
				    TL_TLI_TLISSO ();
				    TL_TLI_TLIPS ((TLint4) 0, __x309, (TLint2) -1);
				    TL_TLI_TLIPK ((TLint2) -1);
				};
				if (k == __x308) break;
				k++;
...
static void getprogramarguments () {
    ...
    {
	TLstring	__x310;
	TL_TLI_TLIFA((TLint4) 1, __x310);
	TLSTRASS(255, pcfile, __x310);
    };
    {
	TLstring	__x311;
	TL_TLI_TLIFA((TLint4) 2, __x311);
	TLSTRASS(255, npcfile, __x311);
    };
    if (TL_TLI_TLIARC > 2) {
	{
	    TLstring	__x312;
	    TL_TLI_TLIFA((TLint4) 3, __x312);
	    threshold = TL_TLS_TLSVS8(__x312);
	};
	...
	{
	    TLstring	__x313;
	    TL_TLI_TLIFA((TLint4) 4, __x313);
	    minclonelines = TL_TLS_TLSVSI(__x313, (TLint4) 10);
	};
	...
	{
	    TLstring	__x314;
	    TL_TLI_TLIFA((TLint4) 5, __x314);
	    maxclonelines = TL_TLS_TLSVSI(__x314, (TLint4) 10);
	};
...
void TProg () {
    ...
    nfragments = 0;
    depth = 0;
    __x278 = TLDYN(maxclonelines - -1);
    __x279 = TLDYN(maxclonelines - -1);
    TL_TLB_TLBALL((TLint4) __x278 * (__x279 * 2), &dpmatrix);
    cdepth = 0;
    TL_TLI_TLISS ((TLint4) 0, (TLint2) 2);
