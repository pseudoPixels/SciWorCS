...
% Incremental extension, January 2011
...
% crossclones.x pcfile npcfile threshold minclonelines maxcloneslines showsource
var pcfile : string			% name of the system_functions.xml pcs file
var npcfile : string			% name of the new_functions.xml pcs file
...
% Usage message
procedure useerr
    put : 0, "Usage :  crossclones.x pcfile.xml newpcfile.xml [ threshold ] [ minclonesize ] [ maxclonesize ] [ showsource ] > clone-pairs-file.xml"
    put : 0, ""
    put : 0, "  e.g.:  crossclones.x linux_functions.xml new_functions.xml 0.2 > new_functions-clones.xml"
    put : 0, ""
...
procedure findclones 
	...
	if pcs (i).nlines not= 0 then
		...
		% If the second pc is within  our limits (already marked by nlines>0)
			...
			cp.nlines := (pcs (i).nlines + pcs (j).nlines + 1) div 2
		    end if
		end if
	    end for
	end if
...
procedure showpairs
    ...
    put "<clones>"
    put "<systeminfo processor=\"nicad3\" system=\"", systemname, "\" granularity=\"", granularity, "\" threshold=\"", round (threshold * 100 + epsilon), 
