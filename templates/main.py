...
def img_reg():
	...
	for j in range(len(T)):
		io.imsave("static/reg/" + "/OUT_IMG_" + '0' + "_" + str(j + 1) +".jpg", T[j])
...
##################################################################
####################P2IRC SEGMENTATION STARTS ####################
@app.route('/plot_segmentation')
def plot_segmentation():
	num_blocs = 6
	num_plots_per_bloc = 10
	gaps_blocs = np.array([1])
	gaps_plots = np.array([1])
	buffer_blocwise = 0.2
	buffer_plotwise = 0.2
	plot_width = 3
	plot_height = 2
	...
	set_plot_layout_relative_meters(buffer_blocwise_m, plot_width_m,gaps_blocs_m,num_plots_per_bloc, buffer_plotwise_m, plot_height_m, gaps_plots_m,num_blocs)
	return render_template('p2irc_apis.html')
